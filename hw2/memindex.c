...
int MIAddPostingList(MemIndex index, char *word, DocID_t docid,
  ...
  if (res == 0) {
    ...
    Verify333(wds != NULL);
    wds->word = word;
    HashTable ht = AllocateHashTable(128);
    Verify333(ht != NULL);
    wds->docIDs = ht;
    kv.value = wds;
    kv.key = wordkey;
...
LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  ...
  while (HTIteratorPastEnd(iter) == 0) {
    ...
    SearchResultPtr str = (SearchResultPtr) malloc(sizeof(SearchResult));
    Verify333(str != NULL);
    str->docid = (DocID_t) kv.key;
    str->rank = NumElementsInLinkedList(kv.value);
    Verify333(AppendLinkedList(retlist, str));
    HTIteratorNext(iter);
  }
  HTIteratorFree(iter);
  ...
  for (i = 1; i < qlen; i++) {
    ...
    for (j = 0; j < ne; j++) {
      SearchResultPtr temp;
      LLIteratorGetPayload(llit, (void**) &temp);
      res = LookupHashTable(wds->docIDs, temp->docid, &kv);
      Verify333(res != -1);
      if (res == 0) {  // doesn't contain query
        LLIteratorDelete(llit, (LLPayloadFreeFnPtr) free);
      } else {
        temp->rank += NumElementsInLinkedList(kv.value);
        LLIteratorNext(llit);
      }
    }
    LLIteratorFree(llit);
