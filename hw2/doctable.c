...
DocID_t DTRegisterDocumentName(DocTable table, char *docname) {
  ...
  temp = LookupHashTable(table->docname_to_docid, res, &oldkv);
  Verify333(temp != -1);  // Out of memory or some error
  if (temp == 1) {
    free(docid);
    free(doccopy);
  ...
  *docid = table->max_id;
  ...
  kv.value = doccopy;
  kv.key = *docid;
  Verify333(InsertHashTable(table->docid_to_docname, kv, &oldkv) == 1);
  // STEP 4.
  // Set up the key/value for the docname_to_docid mapping, and
  // do the insert.
  kv.key = res;
  kv.value = docid;
  Verify333(InsertHashTable(table->docname_to_docid, kv, &oldkv) == 1);
  return *docid;
}
