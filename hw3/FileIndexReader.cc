...
FileIndexReader::FileIndexReader(std::string filename,
  ...
  // MISSING:
  Verify333(fread(&header_.magic_number, sizeof(header_.magic_number), 1, file_) == 1);
  Verify333(fread(&header_.checksum, sizeof(header_.checksum), 1, file_) == 1);
  Verify333(fread(&header_.doctable_size, sizeof(header_.doctable_size), 1, file_) == 1);
  Verify333(fread(&header_.index_size, sizeof(header_.index_size), 1, file_) == 1);
  ...
  if (validate) {
    ...
    while (left_to_read > 0) {
      // MISSING:
      HWSize_t bytesRead = fread(buf, 1, 512, file_);
      left_to_read -= bytesRead;
      for (HWSize_t i = 0; i < bytesRead; i++) {
