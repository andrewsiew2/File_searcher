...
HWSize_t WriteIndex(MemIndex mi, DocTable dt, const char *filename) {
  ...
  // MISSING:
  HWSize_t headerSize = WriteHeader(f, dtres, mires);
  if (headerSize == 0) {
    fclose(f);
    unlink(filename);
    return 0;
...
static HWSize_t WriteDocidDocnameFn(FILE *f,
  ...
  // MISSING:
  //std::cout << (char*)kv->value << std::endl;
  if (fwrite(kv->value, slen_ho, 1, f) != 1) {
    return 0;
  }
  // calculate and return the total amount written.
  // MISSING (change this return to the correct thing):
  return slen_ho + sizeof(header) + sizeof(header.docid);
}
...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  for (i = 0; i < num_pos_ho; i++) {
    ...
    // MISSING:
    LLIteratorGetPayload(it, (void**) &kv2);

    // Truncate to 32 bits, then convert it to network order and write it out.
    // MISSING:
    position.position = (intptr_t) kv2;
    position.position = htonl(position.position);
  ...
  return sizeof(position) * num_pos_ho + sizeof(header);
}
...
static HWSize_t WriteHeader(FILE *f,
  ...
  for (HWSize_t i = 0; i < cslen; i++) {
    uint8_t character; 
    if (fread(&character, sizeof(character), 1, f) != 1) {
      return 0;
    }
    crcobj.FoldByteIntoCRC(character);
  }
  ...
  header.toDiskFormat();
  if (fseek(f, sizeof(header.magic_number), SEEK_SET) != 0) {
    return 0;
  }
  // write stuff
  if (fwrite(&header.checksum, sizeof(header.checksum), 1, f) != 1) {
    return 0;
  }
  if (fwrite(&header.doctable_size, sizeof(header.doctable_size), 1, f) != 1) {
    return 0;
  }
  if (fwrite(&header.index_size, sizeof(header.index_size), 1, f) != 1) {
    return 0;
  }
...
static HWSize_t WriteBucket(FILE *f,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      // MISSING:
      // Writing the element position
      HWSize_t next = htonl(nextelpos);
      // move the file pointer to the place
      ...
      // actual writng
      res = fwrite(&next, sizeof(next), 1, f);
      if (res != 1) {
        LLIteratorFree(it);
        return 0;
      }
      LLIteratorGetPayload(it, (void**) &kv);
      // write element
...
static HWSize_t WriteHashTable(FILE *f,
  ...
  for (i = 0; i < ht->num_buckets; i++) {
    ...
    res = WriteBucketRecord(f, list, next_bucket_rec_offset, next_bucket_offset);
    if (res == 0) {
      return res;
    }
