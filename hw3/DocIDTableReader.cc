...
bool DocIDTableReader::LookupDocID(const DocID_t &docid,
  ...
  for (auto it = elements.begin(); it != elements.end(); it++) {
    ...
    Verify333(fread(&header.num_positions, sizeof(header.num_positions), 1, file_) == 1);
    header.toHostFormat();
    // Is it a match?
    if (header.docid == docid) {
      ...
      for (HWSize_t i = 0; i < header.num_positions; i++) {
        DocPositionOffset_t temp;
        Verify333(fread(&temp, sizeof(temp), 1, file_) == 1);
        temp = ntohl(temp);
        list.push_back(temp); 
      }
...
list<docid_element_header> DocIDTableReader::GetDocIDList() {
  ...
  for (HWSize_t i = 0; i < header_.num_buckets; i++) {
    ...
    // the index file .
    // MISSING:
    Verify333(fseek(file_, offset_ + sizeof(BucketListHeader) + sizeof(bucket_rec) * i, SEEK_SET) == 0);
    ...
    Verify333(fread(&b_rec.bucket_position, sizeof(b_rec.bucket_position), 1, file_) == 1);
    b_rec.toHostFormat();
    ...
    for (HWSize_t j = 0; j < b_rec.chain_len; j++) {
      ...
      // MISSING:
      Verify333(fread(&(ep.element_position), sizeof(ep.element_position), 1, file_) == 1);
      ep.toHostFormat();
      ...
      // MISSING:
      Verify333(fread(&(doc_el.docid), sizeof(doc_el.docid), 1, file_) == 1);
      Verify333(fread(&(doc_el.num_positions), sizeof(doc_el.num_positions), 1, file_) == 1);
      doc_el.toHostFormat();
