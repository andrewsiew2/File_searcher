...
HashTableReader::HashTableReader(FILE *f, IndexFileOffset_t offset)
  ...
  Verify333(fread(&header_.num_buckets, sizeof(header_.num_buckets), 1, file_) == 1);
...
HashTableReader::LookupElementPositions(HTKey_t hashKey) {
  ...
  IndexFileOffset_t element_positions;
  Verify333(fseek(file_, b_rec.bucket_position, SEEK_SET) == 0);
  for (HTKey_t i = 0; i < b_rec.chain_len; i++) {
    Verify333(fread(&element_positions, sizeof(element_positions), 1, file_) == 1);
    element_positions = ntohl(element_positions);
    retval.push_back(element_positions);
  }
  // Return the list.
  return retval;
}
