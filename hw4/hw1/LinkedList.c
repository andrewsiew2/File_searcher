...
bool PopLinkedList(LinkedList list, LLPayload_t *payload_ptr) {
  ...
  if (list->num_elements == 0) {
    ...
    *payload_ptr = oldHead->payload;
    LinkedListNodePtr newHead = oldHead->next;
    list->head = newHead;
    newHead->prev = NULL;
  }
  free(oldHead);
...
bool SliceLinkedList(LinkedList list, LLPayload_t *payload_ptr) {
  ...
  if (list->num_elements == 0) {
    ...
    *payload_ptr = oldTail->payload;
    LinkedListNodePtr newTail = oldTail->prev;
    newTail->next = NULL;
    list->tail = newTail;
  }
  free(oldTail);
...
bool LLIteratorDelete(LLIter iter,
  ...
  (*payload_free_function) (iter->node->payload);  // Frees the payload
  iter->list->num_elements--;
  free(iter->node);
  if (iter->list->num_elements == 0) {  // Case if the list is empty
    iter->list->head = NULL;
    iter->list->tail = NULL;
    iter->node = NULL;
    return false;
  }
  LinkedListNodePtr head = iter->list->head;
  LinkedListNodePtr tail = iter->list->tail;
  if (iter->node == head) {  // Case if iter points at head
    iter->list->head = iter->node->next;
    iter->node = iter->node->next;
    iter->node->prev = NULL;
  } else if (iter->node == tail) {
