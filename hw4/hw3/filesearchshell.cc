...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    listOfIndexes.push_back(argv[i]);
  }
  QueryProcessor processor(listOfIndexes);
  while (1) {
  	string word;
  	vector<string> query;
    ...
    string indivWords;
    while (stream >> indivWords) {
      query.push_back(indivWords);
    }
    ...
    if (result.size() == 0) {
      ...
      vector<QueryProcessor::QueryResult> :: iterator itr;
	  for (itr = result.begin(); itr != result.end(); itr++) {
	    cout << "  " << itr->document_name << " (" << itr->rank << ")" << endl;
	  }
    }
  }
